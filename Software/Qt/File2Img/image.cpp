...
GraphicBin::GraphicBin(bool isContextualMenu, QWidget * parent) : QGraphicsView(parent)
{
...
void GraphicBin::showContextMenu(const QPoint & pos)
{
...
void GraphicBin::setImage(const QImage & image)
{
...
void GraphicBin::setImageFromRawData(const uchar * data, int width, int height, bool mirrorHorizontally, bool mirrorVertically)
{
    // Convert data into QImage
    QImage image(data, width, height, width, QImage::Format_Grayscale8, 0, 0);
...
void GraphicBin::setSize(int width, int height)
{
    pixmapItem->setPixmap(pixmap.scaled(width, height));
}

QPixmap GraphicBin::getPixmap(void)
{
    return pixmap;
}

QSize GraphicBin::getImageSize(void)
{
    return imageSize;
}

// Fit the image in the widget
void GraphicBin::fitImage()
{
...
void GraphicBin::mousePressEvent(QMouseEvent *event)
{
...
// Called when a mouse button is pressed
void GraphicBin::mouseReleaseEvent(QMouseEvent *event)
{
...
void GraphicBin::wheelEvent(QWheelEvent *event)
{
    // When zooming, the view stay centered over the mouse
    this->setTransformationAnchor(QGraphicsView::AnchorUnderMouse);
...
void GraphicBin::mouseMoveEvent(QMouseEvent *event)
{
...
void GraphicBin::drawForeground(QPainter *painter, const QRectF&)
{
...
void GraphicBin::resizeEvent(QResizeEvent *event)
{
...
QString GraphicBin::setToolTipText(QPoint imageCoordinates)
{
...
// Define the virtual function to avoid the "unused parameter" warning
void GraphicBin::drawOnImage(QPainter* , QSize )
{}

// Define the virtual function to avoid the "unused parameter" warning
void GraphicBin::drawInViewPort(QPainter* , QSize )
{}
